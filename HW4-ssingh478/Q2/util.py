...
def entropy(class_y):
    ...
    n0=class_y.count(0)
    n1=class_y.count(1)
    p0=n0/(n0+n1) if n0!=0 else 1
    p1=n1/(n0+n1) if n1!=0 else 1 
    entropy = (p0*(np.log(p0)/np.log(2)))+(p1*(np.log(p1)/np.log(2)))
    return -1*entropy
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    ''' 
    #X=np.array(X)
    xv=[x[split_attribute] for x in X]
    xv=np.array(xv)
    if xv.dtype.name!='str64':
        X_left=[x for x in X if x[split_attribute]<=split_val]
        X_right=[x for x in X if x[split_attribute]>split_val]
        y_left=[y[i] for i,x in enumerate(X) if x[split_attribute]<=split_val]
        y_right=[y[i] for i,x in enumerate(X) if x[split_attribute]>split_val]
