...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        # XX=np.array(XX)
        # sampar=range(n)
        # samples_index = np.random.choice(sampar, size=sampar.shape, replace=True) # sampled dataset
        # samples=XX[samples_index,:]
        # #XX=np.array(XX)
        # # samples = np.random.choice(XX, size=XX.shape, replace=True) # sampled dataset
        # labels  = samples[:,-1]  # class labels for the sampled records
        # samples = samples[:,:-1]
        samples=[]
        labels=[]

        for i in range(n):
            sam=XX[np.random.randint(0,n)]
            samples.append(sam[:-1])
            labels.append(sam[-1])
